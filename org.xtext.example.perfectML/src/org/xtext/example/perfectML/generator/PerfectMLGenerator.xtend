...
//		fsa.generateFile('greetings.txt', 'People to greet: ' + resource.allContents.toIterable().head.compile())
	...
	def dispatch String compileParams(SVC algoSVC) {
		'''
			«IF algoSVC.c_def»
				clf.set_params(c=«algoSVC.c.compile()»)
			«ENDIF»
			...
			«IF algoSVC.coef0_def»
				clf.set_params(coef0=«algoSVC.coef0.compile()»)
			«ENDIF»
			«IF algoSVC.shrinking_def»
				clf.set_params(shrinking=«algoSVC.shrinking.compile()»)
			«ENDIF»
			«IF algoSVC.probability_def»
				clf.set_params(probability=«algoSVC.probability.compile()»)
			«ENDIF»
			«IF algoSVC.tol_def»
				clf.set_params(tol=«algoSVC.tol.compile()»)
			«ENDIF»
			«IF algoSVC.cache_size_def»
				clf.set_params(cache_size=«algoSVC.cache_size.compile()»)
			«ENDIF»
			«IF algoSVC.verbose_def»
				clf.set_params(verbose=«algoSVC.verbose.compile()»)
			«ENDIF»
			...
			«IF algoSVC.break_ties_def»
				clf.set_params(break_ties=«algoSVC.break_ties.compile()»)
			«ENDIF»
	...
	def dispatch String compileParams(MLPClassifier algoMLP) {
		'''
			...
			«IF algoMLP.alpha_def»
				clf.set_params(alpha=«algoMLP.alpha.compile()»)
			«ENDIF»
			...
			«IF algoMLP.learning_rate_init_def»
				clf.set_params(learning_rate_init=«algoMLP.learning_rate_init.compile()»)
			«ENDIF»
			«IF algoMLP.power_t_def»
				clf.set_params(power_t=«algoMLP.power_t.compile()»)
			«ENDIF»
			«IF algoMLP.max_iteer_def»
				clf.set_params(max_iter=«algoMLP.max_iter.compile()»)
			«ENDIF»
			«IF algoMLP.shuffle_def»
				clf.set_params(shuffle=«algoMLP.shuffle.compile()»)
			«ENDIF»
			«IF algoMLP.random_state_def»
				clf.set_params(random_state=«algoMLP.random_state»)
			«ENDIF»
			«IF algoMLP.tolerance_def»
				clf.set_params(tolerance=«algoMLP.tolerance.compile()»)
			«ENDIF»
			«IF algoMLP.verbose_def»
				clf.set_params(verbose=«algoMLP.verbose.compile()»)
			«ENDIF»
			«IF algoMLP.warm_start_def»
				clf.set_params(warm_start=«algoMLP.warm_start.compile()»)
			«ENDIF»
			«IF algoMLP.momentum_def»
				clf.set_params(momentum=«algoMLP.momentum.compile()»)
			«ENDIF»
			«IF algoMLP.nesterovs_momentum_def»
				clf.set_params(nesterovs_momentum=«algoMLP.nesterovs_momentum.compile()»)
			«ENDIF»
			«IF algoMLP.early_stopping_def»
				clf.set_params(early_stopping=«algoMLP.early_stopping.compile()»)
			«ENDIF»
			«IF algoMLP.validation_fraction_def»
				clf.set_params(validation_fraction=«algoMLP.validation_fraction.compile()»)
			«ENDIF»
			«IF algoMLP.beta_1_def»
				clf.set_params(beta_1=«algoMLP.beta_1.compile()»)
			«ENDIF»
			«IF algoMLP.beta_2_def»
				clf.set_params(beta_2=«algoMLP.beta_2.compile()»)
			«ENDIF»
			«IF algoMLP.epsilon_def»
				clf.set_params(epsilon=«algoMLP.epsilon.compile()»)
			«ENDIF»
	...
	def dispatch compile(BOOL b) {
		b.value
	}

	def dispatch compile(FLOAT f) {
		f.intPart + "." + f.decPart
	}
